...
public class LRTATactico : SeekAcelerado
{
    [SerializeField] GameObject suelo = null;
    [SerializeField] GameObject mapaInfluencia = null;
    private MapaInfluencia mapaCostesTacticos;
    ...
    private void Start()
    {
        mapaCostesTacticos = mapaInfluencia.GetComponent<MapaInfluencia>();

        switch (colorEnemigo)
        {
            case ColorEnemigo.Rojo: { Color = Color.red; break; }
            case ColorEnemigo.Azul: { Color = Color.blue; break; }
        }
    ...
    public MovimientoAcelerado getSteering(GameObject target, GameObject personaje, float maxVelocidad, float maxAceleracion, float maxVelAngular, float maxAceleracionAngular, float orientacion, Vector3 velocidad)
    {
        ...
        if (cTargetAnterior.x != Mathf.Infinity && (cTargetAnterior.x != cTarget.x || cTargetAnterior.y != cTarget.y)) { inicializar = true; reestablecido = true; }// Ha cambiado el target
        ...
        if (casillaActualAnterior.x != Mathf.Infinity && (casillaActualAnterior.x == casillaActual.x && casillaActualAnterior.y == casillaActual.y) && !reestablecido )
        {
            // Hasta no llegar a otra casilla no se vuelve a calcular todo
            MovimientoAcelerado baseSeekCasilla = base.getSteering(casillaObjetivo, personaje, maxVelocidad, maxAceleracion, maxVelAngular, maxAceleracionAngular, orientacion, velocidad);

            MovimientoAcelerado resultadoCasilla = new MovimientoAcelerado
            {
                steeringAngular = Vector3.SignedAngle(getVectorOrientacion(personaje, orientacion), velocidad.normalized, Vector3.up),
                steeringLineal = baseSeekCasilla.steeringLineal,
            };
        ...
        if (casillaActualAnterior.x != Mathf.Infinity && ( Mathf.Abs(casillaActualAnterior.x - casillaActual.x) > 1 || Mathf.Abs(casillaActualAnterior.y - casillaActual.y) > 1) ) inicializar = true; // He respauneado o movido al jugador
        ...
        {
            if (
                    map[(int)Mathf.Round(casilla.x), (int)Mathf.Round(casilla.y)] + mapaCostesTacticos.costeTactico((int)Mathf.Round(casilla.x), (int)Mathf.Round(casilla.y), Color)
                            <
                    map[(int)Mathf.Round(casillaDestino.x), (int)Mathf.Round(casillaDestino.y)]  + mapaCostesTacticos.costeTactico((int)Mathf.Round(casillaDestino.x), (int)Mathf.Round(casillaDestino.y), Color)
                )
        ...
        MovimientoAcelerado baseSeek = base.getSteering(casillaObjetivo, personaje, maxVelocidad, maxAceleracion, maxVelAngular, maxAceleracionAngular, orientacion, velocidad);

        MovimientoAcelerado resultado = new MovimientoAcelerado
        {
            steeringAngular = Vector3.SignedAngle(getVectorOrientacion(personaje, orientacion), velocidad.normalized, Vector3.up),
            steeringLineal = baseSeek.steeringLineal,
        };
