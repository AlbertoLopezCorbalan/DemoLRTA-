...
public class Cohesion : SeekAcelerado
{
    //GameObject que contiene todos los compañeros
    [SerializeField]
    GameObject targets = null;

    private GameObject pseudoObjetivo; // Nuevo target donde tenemos que representa el centro de masas.
    ...
    public override MovimientoAcelerado getSteering(GameObject target, GameObject personaje, float maxVelocidad, float maxAceleracion, float maxVelAngular, float maxAceleracionAngular, float orientacion, Vector3 velocidadJugador)
    {
        ...
        Vector3 nuevoObjetivo = new Vector3(centerOfMass.x, 0, centerOfMass.z);
        pseudoObjetivo.transform.position = nuevoObjetivo;

        return base.getSteering(pseudoObjetivo, personaje, maxVelocidad, maxAceleracion, maxVelAngular, maxAceleracionAngular, orientacion, velocidadJugador);
